...
var SyntaxCheckerMulti = function (_xmlData,_dataObject)
{
	...
	temp +='<div id="showcontainer_multi"><input type="submit" id="show" value="Show Me" /></div>';
	temp +='<div id="showallcontainer_multi"><input type="submit" id="showall" value="Show All" /></div>';
	temp +='<div></div>';
	temp +='<div></div>';
	temp +='</div>';
	temp +='</div>';
...
$("#showall").live("click",function(){
	...
	if($(Data).find("existingcommands").length > 0)
		{
		 str = "<div id='ec' class='txtareaLeft_multi'>"+$(Data).find("existingcommands").text()+"</div>";
		}
